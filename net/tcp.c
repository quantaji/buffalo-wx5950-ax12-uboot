 ...
 * General Desription:
 *
 * TCP support imported from upstream U-Boot commit c29c5ed2966425a14aab6794e739a78ab46178a5.
 *
 * HTTP/TCP Receiver:
 *
...
#ifdef CONFIG_TCP_RCV_WND_SIZE
#define TCP_RCV_WND_SIZE	CONFIG_TCP_RCV_WND_SIZE
#else
#if PKTBUFSRX != 0
#define TCP_RCV_BUFFER_SIZE	(PKTBUFSRX * TCP_MSS)
#else
#define TCP_RCV_BUFFER_SIZE	(4 * TCP_MSS)
#endif
#define TCP_RCV_WND_SIZE	((TCP_RCV_BUFFER_SIZE > 0xffffU) ? \
				 0xffffU : TCP_RCV_BUFFER_SIZE)
#endif
#define TCP_MAX_SACK_LEN	(TCP_OPT_LEN_2 + \
				 (TCP_SACK_HILLS - 1) * TCP_OPT_LEN_8)
 ...
 */
static uint tcp_random_port(void)
{
...
static void tcp_send_repeat(struct tcp_stream *tcp)
{
	...
	if (tcp->retry_tx_len > 0) {
		tcp_opts_size = tcp->lost.len;
		if (tcp_opts_size > TCP_MAX_SACK_LEN)
			tcp_opts_size = TCP_MAX_SACK_LEN;
...
static void tcp_steam_tx_try(struct tcp_stream *tcp)
{
	...
	if (tcp_opts_size > TCP_MAX_SACK_LEN)
		tcp_opts_size = TCP_MAX_SACK_LEN;
	tcp_opts_size = ROUND_TCPHDR_BYTES(TCP_TSOPT_SIZE + tcp_opts_size);
...
{
	u32 sack_len = tcp->lost.len;

	if (sack_len > TCP_MAX_SACK_LEN)
		sack_len = TCP_MAX_SACK_LEN;
	...
	if (IS_ENABLED(CONFIG_PROT_TCP_SACK)) {
		if (sack_len > TCP_OPT_LEN_2) {
			...
			/*
			 * Four hills are retained internally, but a timestamped
			 * TCP header has room for only three SACK blocks.
			 */
...
void net_set_syn_options(struct tcp_stream *tcp, union tcp_build_pkt *b)
{
	...
	tcp->loc_timestamp = (u32)get_ticks();
	b->ip.t_opt.t_snd = htonl(tcp->loc_timestamp);
	b->ip.t_opt.t_rcv = tcp->rmt_timestamp;
	b->ip.end = TCP_O_END;
}
		       ...
		       u8 action, u32 tcp_seq_num, u32 tcp_ack_num)
{
	...
	b->ip.hdr.ip_len = htons(pkt_len);
	b->ip.hdr.ip_p = IPPROTO_TCP;
	b->ip.hdr.ip_sum = compute_ip_checksum(&b->ip, IP_HDR_SIZE);
...
{
	int i, j, cnt, cnt_move, tail_count;
	...
	for (i = 0; i < cnt; i++) {
		...
		if (cnt_move > 0) {
			tail_count = cnt - i - cnt_move - 1;
			if (tail_count > 0)
				memmove(&tcp->lost.hill[i + 1],
					&tcp->lost.hill[i + cnt_move + 1],
					tail_count * sizeof(struct sack_edges));
...
struct tcp_stream *tcp_stream_connect(struct in_addr rhost, u16 rport)
{
	...
	tcp = tcp_stream_add(rhost, rport, tcp_random_port());
	if (!tcp)
		return NULL;
